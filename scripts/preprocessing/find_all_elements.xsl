...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Use the below to pull all elements from generate TEI files -->
        ...
        <xsl:for-each-group select="collection($collection_location)//tei:div1[@type='letter']//*" group-by="string-join((name(), mf:sort(@*)/concat(name(), '=&quot;', ., '&quot;')), ' ')">
    ...
    <!-- Use the below to pull all elements from generate HTML files -->
        <xsl:variable name="collection_location">../../output/development/html</xsl:variable>
